...
public class ImageResizePlugin extends CordovaPlugin {
	...
	private boolean resizeImage(JSONObject params, String format, Bitmap bmp, CallbackContext callbackContext) {
	    try {
			double widthFactor;
			double heightFactor;
			...
			if (params.getBoolean("storeImage")) {
			    try {
    			    ...
    			    Log.d("PLUGIN", "storing image");
    			    Log.d("PLUGIN", params.getString("directory"));
    			    URI uri = new URI(params.getString("directory") + "/" + filename);
    			    Log.d("PLUGIN", uri.toString());
    			    File file = new File(uri);
    			    file.mkdirs();
    			    OutputStream outStream = new FileOutputStream(file);
    				if (format.equals(FORMAT_PNG)) {
    					resized.compress(Bitmap.CompressFormat.PNG, quality,
    							outStream);
    				} else {
    					resized.compress(Bitmap.CompressFormat.JPEG, quality,
    							outStream);
    				}
    				outStream.flush();
    				outStream.close();
	...
	private boolean storeImage(JSONObject params, String format, Bitmap bmp, CallbackContext callbackContext) {
	    try {
	...
	private Bitmap getBitmap(String imageData, String imageDataType)
		...
		if (imageDataType.equals(IMAGE_DATA_TYPE_BASE64)) {
		    ...
		    //File imageFile = new File(cordova.getActivity().getFilesDir(), imageData);
		    URI uri = new URI(imageData);
		    File imageFile = new File(uri);
		    Log.d("PLUGIN", imageFile.getAbsolutePath());
